function CShowFullImage(url) {
    ...
    IImage.addEventListener("load", function() {
        ...
        Creators.Actions.Modals.alert({
            name: "Image Preview",
            innerHTML: IImage.outerHTML,
            options: {
                width: `${Math.min(IImage.width, 1000) + 40}px`
            }
        });
    }, false);
    IImage.src = url;
...
document.addEventListener("DOMContentLoaded", () => {
    var copyrightYearField = document.getElementById("copyright-year-field");
    var date = new Date();
    var currentYear = date.getFullYear();
    copyrightYearField.innerText = currentYear;
});